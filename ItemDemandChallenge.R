...
# Everything beyond this is contained more fully in
# the Kaggle Notebook for this competition
...
prophet_model_process <- function(store_item_train, store_item_test) {
  # Define CV split
  prophet_split <- time_series_split(store_item_train, assess="3 months", cumulative = TRUE)
  ...
  # Evaluate accuracy
  # prophet_results %>% modeltime_accuracy() %>% 
  #   table_modeltime_accuracy(.interactive = FALSE)
  
  # # Refit to all data then forecast
  prophet_fullfit <- prophet_results %>% 
    modeltime_refit(data = store_item_train)
  prophet_preds <- prophet_fullfit %>% 
    modeltime_forecast(h = "3 months", 
                       new_data = store_item_test, 
                       actual_data = store_item_train) %>% 
    rename(date = .index, sales = .value) %>% 
    select(date, sales) %>% 
    full_join(., y = store_item_test, by = "date") %>% 
    select(id, sales)
  first_index <- length(store_item_train$date) + 1
  true_id <- prophet_preds$id[first_index:length(prophet_preds$id)]
  true_sales <- prophet_preds$sales[first_index:length(prophet_preds$sales)]
  true_prophet_preds <- data.frame("id" = true_id, "sales" = true_sales)
  
  # Return the predictions
  true_prophet_preds
}
prophet_model_process(filtered_demand, filtered_test)

# Boosted trees

# Miscellaneous processing
unique_stores <- demand_train$store %>% unique()
unique_stores
unique_items <- demand_train$item %>% unique()
unique_items

all_combos <- expand_grid(unique_stores, unique_items) %>% mutate(combined = paste0(unique_stores, " ", unique_items))
all_combos
num_sampled <- 10
si_sample <- sample(all_combos$combined, num_sampled)
si_sample
split_combos <- str_split(si_sample, " ")
split_combos
store_sample <- list_transpose(split_combos)[[1]]
item_sample <- list_transpose(split_combos)[[2]]
si_combos <- data.frame("store" = store_sample, "item" = item_sample)
si_combos
